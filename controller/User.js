...
router.get('/getNewborn', async (req, res) => {
  try {
    ...
    if (!id || !userId) { // Check if either id or userId is falsy (empty or undefined)
      return res.status(400).json({ message: 'ID and UserID are required' });
    }

    const user = await Newborn.findOne({ _id: id, Userid: userId });
    if (!user) {
...
router.get('/getChild' , async(req, res)=>{
  try{
    const { id, userId } = req.query;
    if(!id || !userId){
      return res.status(400).json({message:'Id and UserID are reuired'});
    }
    const user = await Childrens.findOne({_id:id , Userid: userId});
    if(!user){
      return res.status(404).json({message: 'user not found'});
    }
    return res.status(200).json({message:'data fetched successfully' , user});
...
router.get('/youthDetails' , async(req,res)=>{
  try{
    const { id, userId } = req.query;
    if(!id || !userId){
      return res.status(404).json({message:'id and UserID are  required'});   
    }
    const user = await Youth.findOne({_id:id, Userid: userId});
    if(!user){
...
router.get('/middleageDetails', async(req,res)=>{
  try{
    ...
    const user = await Middleage.findOne({_id:id , Userid: userId});
    if(!user){
      return res.status(401).json({message:'user not found'});
    }
    return res.status(200).json({message:'data fetched successfully' ,user});
...
router.get('/getSeniorCitizen' ,async( req,res) =>{
  try{
    const { id, userId } = req.query;
    if(!id || !userId){
      returnres.status(404).json({message:'id and UserID are required'});
    }
    const user = await SeniorCitizen.findOne({_id:id , Userid: userId});
    if(!user){
      return res.status(401).json({message:'user not found'});
    }
    return res.status(200).json({message:'data fetched successfully' , user});
...
router.get('/getSuperCitizen' ,async( req,res) =>{
  try{
    ...
    const user = await Supercitizen.findOne({_id:id ,Userid: userId});
    if(!user){
      return res.status(401).json({message:'user not found'});
    }
    return res.status(200).json({message:'data fetched successfully' , user});
